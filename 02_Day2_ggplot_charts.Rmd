...
my_cat <- mutate(my_cat, 
                 found_by = "Secret Agent, shhhhh...")

```

### <i class="fa fa-user-secret" aria-hidden="true" style="color:#040707;"></i> Pro-tip! {-}
> If you use `mutate()` and provide a single value for a column such as `found_by = "Secret Agent"`, then every row in the column you created will have that value. If you provide a vector of values, such as `human_age = cat_age * 4 + 20`, then the column you created will have values corresponding to the values in the vector you provided. If you provide a vector that has a different length than the number of rows in your data frame, you'll get an error telling you the number of values you provide must be equal to the number of rows in your data frame or one.
...
# Path to movie data
movie_path <- "X:/Agency_Files/Outcomes/Risk_Eval_Air_Mod/_Air_Risk_Evaluation/R/R_Camp/Student Folder/Sample data/IMDB.csv"


# Load movie data
movies <- read_csv(movie_path)
...
```{r, eval = F, echo = F}
## Ignore the script below. It's for the buttons. #############################
```
<script>
